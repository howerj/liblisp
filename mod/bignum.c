...
static void adjust_last(bignum * n) { /*clean up after operation*/
        assert(n);
...
static int leftshift(bignum *n, unsigned d) { /*Multiply a bignum by radix^d*/
        ...
        if((0 == n->lastdigit) && (0 == n->digits[0])) return 0;
        if(!(p = realloc(n->digits,n->allocated+d))) return -1;
...
bignum *bignum_strtobig(const char *s, unsigned int base) {
        ...
        if(!s || (MAX_RADIX < base)) return NULL;

        len = strlen(s);

        if(!(n = bignum_create(0,len+1))) return NULL;
...
int bignum_compare(bignum *a, bignum *b) {
        ...
        if((a->isnegative == 1) && (b->isnegative == 0)) return -1;
        if((a->isnegative == 0) && (b->isnegative == 1)) return  1;

        if(a->lastdigit < b->lastdigit) return a->isnegative ?  1 : -1;
        if(a->lastdigit > b->lastdigit) return a->isnegative ? -1 :  1;
...
bignum *bignum_add(bignum *a, bignum *b) {
        ...
        if(!(result = bignum_create(0, allocate))) return NULL;
...
bignum *bignum_subtract(bignum *a, bignum *b) {
        ...
        if(!(result = bignum_create(0, a->lastdigit + 1))) return NULL;
...
bignum *bignum_multiply(bignum *a, bignum *b) {
        ...
        if(!(result = bignum_create(0, allocate))) return NULL;
        if(!(row = bignum_create(0, allocate))) return NULL;
        ...
        for (i = 0; i <= b->lastdigit; i++) {
                for(j = 0; j < b->digits[i]; j++) {
                        tmp = result;
                        if(!(result = bignum_add(result, row))) return NULL;
                        bignum_destroy(tmp);
                }
                if(leftshift(row,1) < 0) return NULL;
        }
        bignum_destroy(row);
...
bignum_div_t *bignum_divide(bignum *a, bignum *b) {
        ...
        if(!(quotient = bignum_create(0, a->lastdigit + 1))) goto fail;
        if(!(row = bignum_create(0, a->lastdigit + 1))) goto free_quotient;

        /*quotient is zero at this point*/
        if(bignum_compare(b,quotient) == 0) goto free_row;
        ...
        do {    i--;
                ...
                while(-1 < bignum_compare(row, b)) {
                        quotient->digits[i]++;
                        if(!(tmp = bignum_subtract(row, b))) goto free_row;
                        bignum_copy(row,tmp);
                        bignum_destroy(tmp);
                }
        } while (i > 0);
        ...
        if(!(result = malloc(sizeof (*result)))) goto free_row;
