...
static int ud_sql_print(io *o, unsigned depth, cell *f) {
        return lisp_printf(NULL, o, depth, 
                        "%B<SQL-STATE:%d:%s>%t", 
                                get_user(f), lisp_is_cell_closed(f) ? "CLOSED" : "OPEN");
}
...
static int initialize(void) {
        ...
        if(ud_sql < 0)
                goto fail;
        if(!lisp_add_subr(lglobal, "sql",       subr_sql, NULL, NULL)) goto fail;
        if(!lisp_add_subr(lglobal, "sql-open",  subr_sqlopen, NULL, NULL)) goto fail;
        if(!lisp_add_subr(lglobal, "sql-close", subr_sqlclose, NULL, NULL)) goto fail;
